...
module game_fsm (
    ...
    reg [9:0] player_w;
    reg [2:0] level;   // counts how many times difficulty increased
        ...
        if (rst) begin
            ...
            ST_PLAY: begin
                if (tick) begin
                    ...
                    // Paddle
                    if (ax_filt > DEAD_ZONE || ax_filt < -DEAD_ZONE)
                    ...
                    // Walls
                    if (nx <= BORDER) begin nx = BORDER; tmp_dx = -tmp_dx; end
                    if (nx + BALL_W >= SCREEN_W - BORDER) begin
                        nx = SCREEN_W - BORDER - BALL_W;
                        tmp_dx = -tmp_dx;
                    end
                    if (ny <= BORDER) begin ny = BORDER; tmp_dy = -tmp_dy; end
                    ...
                    // Clamp
                    if (tmp_dx >  MAX_SPEED_X) tmp_dx =  MAX_SPEED_X;
                    if (tmp_dx < -MAX_SPEED_X) tmp_dx = -MAX_SPEED_X;
                    if (tmp_dy >  MAX_SPEED_Y) tmp_dy =  MAX_SPEED_Y;
                    if (tmp_dy < -MAX_SPEED_Y) tmp_dy = -MAX_SPEED_Y;
